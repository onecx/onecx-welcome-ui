...
describe('ImageCreateComponent', () => {
  ...
  describe('save', () => {
    it('should prevent save image if form is invalid', () => {
      component.formGroup.controls['url'].setValue('someUrl')
      component.ngOnInit()
      fixture.detectChanges()
      apiServiceSpy.createImageInfo.and.returnValue(of({ id: '123', url: 'someUrl', position: '1' } as ImageInfo))
      component.formGroup.controls['url'].setErrors({ required: true }) // make form invalid

      component.onSave()

      expect(msgServiceSpy.success).not.toHaveBeenCalled()
      component.formGroup.controls['url'].setErrors(null) // make form valid
    })

    it('should save image with url', () => {
      component.formGroup.controls['url'].setValue('https://example.com/image.png')
      component.ngOnInit()
      fixture.detectChanges()
      apiServiceSpy.createImageInfo.and.returnValue(of({ id: '123', url: 'someUrl', position: '1' } as ImageInfo))
